...
const BenifitsSection = () => {
  const benifits = APP_CONFIG.BENEFITS;
  return (
    <section className='py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-gray-50 to-gray-100'>
      
      <div className='max-w-7xl mx-auto'>
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-12 itms-center'>
            ...
            <ul className='space-y-4'>
              {benifits.map((benifit, index) => (
                <li key={index} className='flex items-center'>
                  <FaCheckCircle className='w-6 h-6 text-green-500 mr-3 mt-1 shrink-0'/>
                  <span className='text-lg text-gray-700'>{benifit}</span>
                </li>
              ))}
            </ul>
          </div>
